...
Self-contained PowerShell WPF utility for diagnosing and repairing Windows PnP device and driver issues. No external modules are required.

**Repository:** https://github.com/ps81frt/driverRep  
**Latest release:** [v5.3](https://github.com/ps81frt/driverRep/releases/tag/V5.3)
...
- [Tabs](#tabs)
  - [Tab 1 — Device Scan](#tab-1--device-scan)
  - [Tab 2 — Driver Store](#tab-2--driver-store)
  - [Tab 3 — Event Log](#tab-3--event-log)
  - [Tab 4 — Windows Update (Driver Updates)](#tab-4--windows-update-driver-updates)
  - [Tab 5 — System Tools](#tab-5--system-tools)

---

## Overview

A compact PowerShell/WPF tool that scans device and driver state, analyzes the Windows Driver Store, correlates driver-related events, and exposes repair workflows for devices, driver packages, and system health.

## Key points

- Single-file script: `DRTPro4.ps1`
- Auto-elevates when required
- Requires a valid `.ps1` path, not paste-in execution
- Uses WPF with a `DispatcherTimer` + `ConcurrentQueue` pattern for background processing
- Scans PnP devices and matches drivers via `Win32_PnPSignedDriver`
- Parses `pnputil /enum-drivers` output with locale-safe regex
- Includes simulation mode for Driver Store cleanup preview
- Supports SFC, DISM, USB reset, event log correlation, and HTML report export

## Notes

- Run via `powershell.exe -ExecutionPolicy Bypass -File DRTPro4.ps1`.
- The tool requires a script path and cannot be pasted directly into an interactive console.
- Administrator privileges are requested automatically if needed.
...
Calls `pnputil /enum-drivers` and parses the output by detecting `oem\d+.inf` blocks. The parser extracts localized labels with regex, handling `provider|fournisseur`, `version`, `classe|class`, and `original|origine`.
...
- Date
- Class
- Estimated size from `%SystemRoot%\System32\DriverStore\FileRepository`
- Score (`SAFE`, `WARNING`, `CRITICAL`)
- Score reason

Driver sizes are estimated by scanning `%SystemRoot%\System32\DriverStore\FileRepository`, matching each directory to the INF name, and summing file lengths.

Driver removal uses `Invoke-RemoveDrivers` and `pnputil /delete-driver <oemX.inf> /force`, after confirming the package name matches `^oem\d+\.inf$` and requesting user confirmation. The Driver Store list is refreshed after successful removal.

A "Mode Simulation" checkbox is available in the UI and a preview button forces simulation mode. When enabled, the tool logs `[SIM]` actions and estimated free space without executing any `pnputil` commands.
...
```
driverRep/
├── DRTPro.ps1          # Main script (also released as DRTPro4.ps1 in v5.3)
